// Package filegatewayclient 提供数据分析服务访问基础平台文件网关的本地 HTTP 适配器。
...
// TokenSource 返回数据分析服务已获取的文件网关 Bearer 令牌。
type TokenSource func(context.Context) (string, error)

// Client 是数据分析服务独立的文件网关访问适配器。
type Client struct {
	baseURL    string
...
// New 创建客户端并验证平台 API origin 与令牌来源。
...
// Upload 上传数据分析附件并返回文件 ID；requestID 用于完整请求哈希幂等。
...
// Bind 将已就绪文件绑定到数据分析资源，资源归属由业务服务先行校验。
